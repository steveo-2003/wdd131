...
// Function to create a movie article element using DOM methods
function createMovieArticle(movie) {
    const article = document.createElement('article');
    article.className = 'movie';
    
    const starCount = movie.stars.length / 2;
    ...
    article.innerHTML = `
        ...
        <p><strong>Rating:</strong> <span aria-label="${starCount} out of 5 stars">${movie.stars}</span></p>
...
// Loop through each movie and add it to the page
movies.forEach(movie => {
    movieList.appendChild(createMovieArticle(movie));
});